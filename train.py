...
from fairseq.data import add_plain
...
def train(args, trainer, task, epoch_itr, use_plain=False,
        src_plain_epoch_iter=None, trg_plain_epoch_iter=None):
    """Train the model for one epoch."""

    # Initialize data iterator
    itr = epoch_itr.next_epoch_itr()
    src_plain_itr = src_plain_epoch_iter.next_epoch_itr() if use_plain else None
    trg_plain_itr = trg_plain_epoch_iter.next_epoch_itr() if use_plain else None
    ...
    for i, sample in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        if use_plain:
            # print("add plain to sample")
            try:
                src_plain = next(src_plain_itr)
            except StopIteration:
                src_plain_itr = src_plain_epoch_iter.next_epoch_itr()
                src_plain = next(src_plain_itr)
            try:
                trg_plain = next(trg_plain_itr)
            except StopIteration:
                trg_plain_itr = trg_plain_epoch_iter.next_epoch_itr()
                trg_plain = next(trg_plain_itr)
            sample = add_plain(sample, src_plain, trg_plain)
...
def validate(args, trainer, task, epoch_itr, subsets, use_plain=False,
        src_plain_epoch_iter=None, trg_plain_epoch_iter=None):
...
def load_checkpoint(args, trainer, epoch_itr, load_optim=True, find_best=False,
                    fix_discriminator=False):
    """Load a checkpoint and replay dataloader to match."""
    os.makedirs(args.save_dir, exist_ok=True)
    checkpoint_path = os.path.join(args.save_dir, args.restore_file)
    if find_best:
        checkpoint_path = os.path.join(args.save_dir, 'checkpoint_best.pt')
    # print("Finding {} ...".format(checkpoint_path))
    if os.path.isfile(checkpoint_path):
        # print("Found {}".format(checkpoint_path))
        extra_state = trainer.load_checkpoint(
                            checkpoint_path, load_optim=load_optim,
                            fix_discriminator=fix_discriminator,
                            )
    ...
    for split in splits:
        if split == 'train':
            ...
            for k in itertools.count():
                ...
                try:
                    task.load_dataset(split_k, combine=True) # used to be False
