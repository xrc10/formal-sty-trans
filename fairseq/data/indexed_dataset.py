...
DEBUG_MODE_LINE_COUNT = 1000 # read this many lines for debug
...
class IndexedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, path, fix_lua_indexing=False, debug=False):
        super().__init__()
...
class IndexedRawTextDataset(IndexedDataset):
    ...
    def __init__(self, path, dictionary, append_eos=True, reverse_order=False, debug=False):
        self.tokens_list = []
        self.lines = []
        self.sizes = []
        self.append_eos = append_eos
        self.reverse_order = reverse_order
        self.read_data(path, dictionary, debug)
    ...
    def read_data(self, path, dictionary, debug=False):
        ...
        print('{} has lines {}, min length {} max length {}'.format(path, len(self.sizes), min(self.sizes), max(self.sizes)))
