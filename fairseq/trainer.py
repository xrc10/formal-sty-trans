...
class Trainer(object):
    ...
    def load_checkpoint(self, filename, load_optim=True, fix_discriminator=False):
        """Load all training state from a checkpoint file."""
        extra_state, optim_history, last_optim_state = \
            utils.load_model_state(filename, self.model)

        if fix_discriminator:
            print("Fixing discriminator ... ")
            self.model.fix_discriminator()
        ...
        try:
            ...
            if sample is not None:
                with torch.no_grad() if eval else contextlib.ExitStack():
                    ...
                    loss, sample_size, logging_output_ = self.task.get_loss(
                        self.model, self.criterion, sample, num_updates=self._num_updates)
