...
@register_task('style_transfer')
class StyleTransferTask(FairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--debug', dest='debug', action='store_true',
                            help='set true to load small amount of data for debug')
        parser.add_argument('--no-debug', dest='debug', action='store_false',
                            help='set false to debug mode')
    ...
    def setup_task(cls, args, **kwargs):
        ...
        src_dict = Dictionary.load(os.path.join(args.data, 'dict.{}.txt'.format(args.source_lang)),
                                    src_lang=args.source_lang, trg_lang=args.target_lang)
        tgt_dict = Dictionary.load(os.path.join(args.data, 'dict.{}.txt'.format(args.target_lang)),
                                    src_lang=args.source_lang, trg_lang=args.target_lang)
        assert src_dict.pad() == tgt_dict.pad()
        assert src_dict.eos() == tgt_dict.eos()
        assert src_dict.unk() == tgt_dict.unk()
        assert src_dict.src_lang_index == tgt_dict.src_lang_index
        assert src_dict.trg_lang_index == tgt_dict.trg_lang_index
        print('| [{}] dictionary: {} types'.format(args.source_lang, len(src_dict)))
        print('| [{}] dictionary: {} types'.format(args.target_lang, len(tgt_dict)))

        return cls(args, src_dict, tgt_dict)

    def load_dataset(self, split, combine=False, aligned=True):
        ...
        def indexed_dataset(path, dictionary, debug=False):
            if self.args.raw_text:
                ...
                return IndexedInMemoryDataset(path, fix_lua_indexing=True, debug=debug)
            return None
        ...
        for k in itertools.count():
            ...
            src_datasets.append(indexed_dataset(prefix + src, self.src_dict, debug=self.debug))
            tgt_datasets.append(indexed_dataset(prefix + tgt, self.tgt_dict, debug=self.debug))
        ...
        else: # concatenate multiple splits
        ...
        if 'plain' in split or (not aligned):
            self.datasets[split] = (
                LanguagePairDataset(
                ...
                LanguagePairDataset(
                    tgt_dataset, tgt_sizes, self.src_dict,
                    left_pad_source=self.args.left_pad_source,
                    max_source_positions=self.args.max_source_positions,
                )
            )
