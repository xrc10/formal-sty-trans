...
class SequenceGenerator(object):
    def __init__(
        ...
        self.trg_lang_index = tgt_dict.trg_lang_index
        self.src_lang_index = tgt_dict.src_lang_index
    ...
    def generate_batched_itr(
        ...
        cuda=False, timer=None, prefix_size=0, to_trg=True
    ):
    ...
    def generate(self, src_tokens, src_lengths, beam_size=None, maxlen=None, prefix_tokens=None, to_trg=True):
        """Generate a batch of translations."""
        with torch.no_grad():
            return self._generate(src_tokens, src_lengths, beam_size, maxlen, prefix_tokens, to_trg)

    def _generate(self, src_tokens, src_lengths, beam_size=None, maxlen=None, prefix_tokens=None, to_trg=True):
        ...
        for model in self.models:
            ...
            if to_trg:
                lang_code = self.trg_lang_index
            else:
                lang_code = self.src_lang_index
            encoder_out = model.encoder(
                insert_lang_code(src_tokens.repeat(1, beam_size).view(-1, srclen), True, lang_code),
